...
- **Palette**: warm off-white paper (`#f8f7f3`) ground, near-black ink (`#26261f`) text, a single cool desaturated blue accent (`oklch(0.52 0.09 250)`) used sparingly — for the current nav pill, links-on-hover, live status, slider thumbs, chart lines. No gradients, no second accent hue.
...
- **Borders & cards**: 1px hairline borders (`--line` / lighter `--line-soft`) in a warm neutral gray, never black. Cards (`.dl-card`, plot panels, controls) get a 14px radius, a hairline border, and a slightly-lighter-than-paper fill (`--card:#fffefb`) — no shadows anywhere, at rest or hover.
